...
- The current implementation covers the shared spatial contract, backend-owned ambient timeline, state-aware route checks, monotonic client clock, passive homepage, authored evening seed, stale-state handling, and local migration checks. Final art production, full occlusion fixtures, character composition, candidate preload/fade reconciliation, and measured release acceptance remain open.
