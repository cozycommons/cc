...
END IF;
END $$;"""]
        # Validate every selected family's history before applying any SQL.
        application_start = len(chunks)
...
END IF; END $$;""", (directory / f"{family}_schema_contract.sql").read_text()]
        application_chunks.extend(chunks[application_start:])
        del chunks[application_start:]
    chunks += application_chunks
    chunks += ["NOTIFY pgrst, 'reload schema';",
               "SELECT pg_advisory_unlock(hashtextextended('cozycommons:migrations', 0));"]
    return "\n".join(chunks) + "\n"
