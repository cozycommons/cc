...
begin
  ...
  foreach column_spec in array array[
    ...
    'dice_games.source_live_match_id', 'dice_games.live_result_state',
  ...
  if exists (
     ...
     or not exists (
       ...
       where conrelid = 'public.dice_games'::regclass
         ...
         and convalidated
         and pg_get_constraintdef(oid) = 'CHECK (((source_live_match_id IS NULL) OR (NOT ranked)))'
     ) then
