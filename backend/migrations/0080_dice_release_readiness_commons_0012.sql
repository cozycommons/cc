-- Advance release attestation to Dice 0080 and Commons 0012.
-- This is forward-only; 0079 is already applied in staging.
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.dice_schema_migrations WHERE version=80)
     OR NOT EXISTS (SELECT 1 FROM public.commons_schema_migrations WHERE version=12) THEN
