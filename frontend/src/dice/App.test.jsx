...
describe('Dice private route auth hydration', () => {
  it('waits for profile registration before mounting a direct live route', () => {
    mocks.auth = {
    ...
    expect(screen.getByText('Live lobby for u1')).toBeInTheDocument();
  });

  it('remounts route state when the signed-in identity changes', () => {
    mocks.auth = {
    ...
    view.rerender(<MemoryRouter initialEntries={['/dice/live']}><Routes><Route path="/dice/*" element={<DiceApp />} /></Routes></MemoryRouter>);
    expect(screen.getByText('Live lobby for u2')).toBeInTheDocument();
  });
