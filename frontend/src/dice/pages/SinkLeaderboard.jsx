...
export default function SinkLeaderboard({ auth }) {
  ...
  useEffect(() => {
    diceApi.getSinkLeaderboard(500, !experimentalEnabled).then(setEntries).catch(() => setEntries([]));
  }, [experimentalEnabled]);

  const ranked = experimentalEnabled
    ? rankLeaderboard(entries, 'sinks')
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="jk-card overflow-hidden">
        ...
        {ranked.map((p) => (
          <LeaderboardRow key={p.user_id} rank={p.rank} profile={p} value={p.sinks} valueLabel="sinks" barMode={experimentalEnabled ? 'count' : undefined} maxValue={maxValue} isCurrentUser={experimentalEnabled && p.user_id === currentUserId} />
        ))}
      </div>
    </div>
  );
